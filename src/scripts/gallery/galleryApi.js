...
export default class GalleryApi {
    getLocationItems = (openNow, onlyFavourites, downloadedItems) => {
        ...
        if (openNow) {
            downloadedItems = downloadedItems.filter(downloadedLocationItems => {
                return false // TODO: Add open now check
            });
        } else if (onlyFavourites) {
            downloadedItems = downloadedItems.filter(downloadedLocationItems => {
                return downloadedLocationItems.favourite
            });
        } else if (openNow && onlyFavourites) {
            downloadedItems = downloadedItems.filter(downloadedLocationItems => {
                return downloadedLocationItems.favourite // TODO: Add open now check
            });
        } 
